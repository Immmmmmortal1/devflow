...
CODEX_SKILLS_ROOT="$TMP_SKILLS" \
  bash "$ROOT/scripts/install-dev-flow.sh" --codex --skills-only --skip-xcodebuild --skip-debugbridge --skip-review >/dev/null

[[ -L "$TMP_SKILLS/dev-flow" ]]
[[ -f "$TMP_SKILLS/environment-health-check/SKILL.md" ]]

bash "$ROOT/scripts/install-dev-flow.sh" --codex --project "$TMP_PROJECT" --skip-xcodebuild --skip-debugbridge --skip-review >/dev/null
